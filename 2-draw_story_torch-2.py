...
def get_session_update_msg():
    config = {
        "modalities": ["text", "audio"],
        "instructions": "你的名字叫豆包，你是一个一次只能讲600字以内的故事助手",
...
while running:
    ...
    # 显示状态文本  
    blit_text1( 36 , text_state,( 150 , 16 ) )
    # 显示标题文本
    # blit_text1( 36 , text_title,( 420 , 16 ) )
    # 显示故事内容
    font1 = pygame.font.SysFont('SimHei', 20)
    blit_text2(screen, text_story, (10, 60), font1, 490)
    # 绘制按钮
    ...
    if button0_key == 1 :    #生成故事
        ...
        print(text_story)
        #生成故事标题和文本
        translation = translate_long_text(text_story)
        summary = summarizer(translation, max_length=20, min_length=5, do_sample=False)
        text_title = translate_long_text(summary[0]['summary_text'],'en','zh')
        print(text_title)
        button0_key = -1
    ...
    if button2_key == 1 :    #图片生成
        ...
        pipe = pipe.to(device)
        image = pipe(summary[0]['summary_text']).images[0]
